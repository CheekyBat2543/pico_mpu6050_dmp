...
int pico_i2c_write(unsigned char slave_addr, unsigned char reg_addr,
    unsigned char length, unsigned char const *data) {
    ...
    #ifdef CHOSEN_I2C1
    num_bytes_read = i2c_write_blocking(i2c1, slave_addr, msg, (length + 1), false);
    #else
    num_bytes_read = i2c_write_blocking(i2c0, slave_addr, msg, (length + 1), false);
    #endif
    if(num_bytes_read == -1) return -1;
    return 0;
}
int pico_i2c_read(unsigned char slave_addr, unsigned char reg_addr,
    unsigned char length, unsigned char *data) {
    /**/
    int num_bytes_read = 0;
    ...
    #ifdef CHOSEN_I2C1
    i2c_write_blocking(i2c1, slave_addr, &reg_addr, 1, true);
    num_bytes_read = i2c_read_blocking(i2c1, slave_addr, data, length, false);
    #else
    i2c_write_blocking(i2c0, slave_addr, &reg, 1, true);
    if(num_bytes_read == -1) return -1;
    num_bytes_read = i2c_read_blocking(i2c0, slave_addr, data, length, false);
    #endif
    if(num_bytes_read == -1) return -1;

    return 0;
}
void pico_get_ms(unsigned long * count) {
    *count = to_ms_since_boot(get_absolute_time());
    return;
}
#define i2c_write pico_i2c_write
#define i2c_read pico_i2c_read
#define delay_ms sleep_ms
#define get_ms pico_get_ms
#else
...
/**
 * @brief       Convert a Q formatted data to float. 
 * Use q = 30 for quaternions.
 * @param[in]   number  Q formatted number.      
 * @param[in]   q       Q format.
